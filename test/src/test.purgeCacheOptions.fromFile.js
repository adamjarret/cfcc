...
module.exports = ({testSync}) => {
    ...
    {
        const displayText = `purgeCacheOptions.fromFile(${path.basename(filePath)}) :: ${name}`;
    ...
    fromFileTest('Extend file in another directory that depends on a relative path', fixturePath('config-d.json'), {
        'token': 'TOKEN_X',
        'email': 'EMAIL_D',
        'zone': 'ZONE_Y'
    });
